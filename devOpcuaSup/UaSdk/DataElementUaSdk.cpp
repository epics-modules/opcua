...
// Helper function to convert a UA_ByteString to a string
// of hex characters, 2 chars per byte.
static int printByteString(const OpcUa_ByteString& bytestring, char* buffer, int buffersize)
{
    int i, l = 0;
    for (i = 0; i < bytestring.Length && l < buffersize-3; i++) {
        l += sprintf(buffer + l, "%02x", bytestring.Data[i]);
    }
    return l;
}
            ...
            if (OpcUa_IsNotGood(stat)) {
                ...
                if (data.type() == OpcUaType_ExtensionObject) {
                    ...
                    if (definition.isUnion()) {
                        ...
                        if (switchValue > 0) {
                            n = static_cast<OpcUa_Int32>(snprintf(value, len, "%s:%s",
    ...
    case ProcessReason::readComplete:
    {
        if (num && value) {
            ...
            if (OpcUa_IsNotGood(stat)) {
                ...
                // Valid OPC UA value, so try to convert
                UaVariant &data = upd->getData();
                if (data.type() == OpcUaType_ByteString) {
                    const OpcUa_Variant* variant = data;
                    elemsWritten = variant->Value.ByteString.Length;
                    if (num < elemsWritten) elemsWritten = num;
                    memcpy(value, variant->Value.ByteString.Data, elemsWritten);
                    memset(value + elemsWritten, 0, num - elemsWritten);
...
DataElementUaSdk::writeScalar (const char *value, epicsUInt32 len, dbCommon *prec)
{
    ...
    { // Scope of Guard G
        ...
        if (sep) {
            localizedText.setLocale(UaByteString(sep - value, (OpcUa_Byte*)value));
            value = sep + 1;
    ...
    if (!incomingData.isArray()) {
        ...
        case OpcUaType_LocalizedText: {
            ...
            for (epicsUInt32 i = 0; i < num; i++) {
                const char* sep = static_cast<const char*>(memchr(value, '|', len));
                if (sep) {
                    UaString(UaByteString(sep - value, (OpcUa_Byte*)value)).copyTo(&arr[i].Locale);
                } else if (i < incoming.length()) {
                    UaString(incoming[i].Locale).copyTo(&arr[i].Locale);
                } else if (i > 0) {
                    UaString(arr[i-1].Locale).copyTo(&arr[i].Locale);
                }
                UaString(UaByteString(sep ? len - (sep+1-value) : len, (OpcUa_Byte*)(sep ? sep+1 : value))).copyTo(&arr[i].Text);
                value += len;
            }
            { // Scope of Guard G
                Guard G(outgoingLock);
        ...
        case OpcUaType_QualifiedName: {
            ...
            for (epicsUInt32 i = 0; i < num; i++) {
                ...
                UaString(UaByteString(sep ? len - (sep+1-value) : len, (OpcUa_Byte*)(sep ? sep+1 : value))).copyTo(&arr[i].Name);
                value += len;
            }
            { // Scope of Guard G
                Guard G(outgoingLock);
